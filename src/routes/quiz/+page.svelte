...
<div class="flex flex-col h-full gap-10">
    ...
    <form method="post" class="grow flex flex-col gap-10" bind:this={formEl} action="?/check_answer">
        <div class="grid { ['grid-cols-2', 'grid-cols-1', 'grid-cols-2'][answersCount-2] } gap-8 grow">
            ...
            <label class="font-bold text-2xl border dark:border-slate-500 hover:bg-primary-200 dark:hover:bg-primary-800/25 transition-all p-8 rounded-xl" style="order: {Math.round(Math.random() * 1000)};" id="answer1" on:change={goCheck}>
                { data.question.answer }

                <input type="checkbox" name="answer" class="hidden" id="answer1" value={ 0 }>
            </label>

            <label class="font-bold text-2xl border dark:border-slate-500 hover:bg-primary-200 dark:hover:bg-primary-800/25 transition-all p-8 rounded-xl" style="order: {Math.round(Math.random() * 1000)};" id="answer2" on:change={goCheck}>
                { data.question.wrongAnswer1 } 

                <input type="checkbox" name="answer" class="hidden" id="answer2"  value={ 1 }>
            </label>

            <label class="{answersCount < 3 ? 'hidden': ''} font-bold text-2xl border dark:border-slate-500 hover:bg-primary-200 dark:hover:bg-primary-800/25 transition-all p-8 rounded-xl" style="order: {Math.round(Math.random() * 1000)};" id="answer3" on:change={goCheck}>
                { data.question.wrongAnswer2 }

                <input type="checkbox" name="answer" class="hidden" id="answer3"  value={ 2 }>
            </label>

            <label class="{answersCount < 4 ? 'hidden': ''} font-bold text-2xl border dark:border-slate-500 hover:bg-primary-200 dark:hover:bg-primary-800/25 transition-all p-8 rounded-xl" style="order: {Math.round(Math.random() * 1000)};" id="answer4" on:change={goCheck}>
